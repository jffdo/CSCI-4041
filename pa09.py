...
def optimize_wings(price_list,num_wings):
    #TODO: Implement this
    #The following line is a placeholder that passes only the first
    #test case: you'll need to change it.
    rev = [0] * (num_wings+1)
    split = [0] * (num_wings+1)
    for j in range(1, num_wings+1):
        ...
        for i in range(1, j+1):
            index = 0
            while(index < len(price_list)-1 and price_list[index].wings < i):
                index += 1
            if best > (price_list[index].price + rev[j-i]) and i <= price_list[index].wings:
                best = (price_list[index].price + rev[j-i])
                split[j] = price_list[index]
        rev[j] = best
    optimal = []
    wings = num_wings
    while(wings > 0):
        optimal.append(split[wings])
        wings -= split[wings].wings
    return optimal
