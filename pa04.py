...
def max_heapify(task_heap, i):
    left = (2 * i) + 1
    right = (2 * i + 1) + 1
    if left <= len(task_heap) - 1 and task_heap[left].priority > task_heap[i].priority:
        largest = left
    else:
        largest = i
    if (
        right <= len(task_heap) - 1
        and task_heap[right].priority > task_heap[largest].priority
    ):
        largest = right
    if largest != i:
        temp = task_heap[i]
        task_heap[i] = task_heap[largest]
        task_heap[largest] = temp
...
def increase_task_priority(task_heap, i, new_priority):
    ...
    while (i > 0) and task_heap[math.floor((i - 1) / 2)].priority < task_heap[
        i
    ].priority:
        temp = task_heap[i]
        task_heap[i] = task_heap[math.floor((i - 1) / 2)]
        task_heap[math.floor((i - 1) / 2)] = temp
        i = math.floor((i - 1) / 2)
