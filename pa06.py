...
def database_insert(database, robot):
    name_index = hash_function(robot.name)
    database.name_table[name_index].append(robot)

    shape_index = hash_function(robot.shape)
    database.shape_table[shape_index].append(robot)

    color_index = hash_function(robot.color)
    database.color_table[color_index].append(robot)
...
def database_search(database, field, value):
    ...
    print(field == "shape")
    if field == "name":
        name_index = hash_function(value)
        for entity in database.name_table[name_index]:
            if entity.name == value:
                output.append(entity)
    elif field == "shape":
        shape_index = hash_function(value)
        for entity in database.shape_table[shape_index]:
            if entity.shape == value:
                output.append(entity)
    elif field == "color":
        color_index = hash_function(value)
        for entity in database.color_table[color_index]:
...
def database_delete(database, field, value):
    robot_list = database_search(database, field, value)
    for entity in robot_list:
        name_index = hash_function(entity.name)
        database.name_table[name_index].remove(entity)

        shape_index = hash_function(entity.shape)
        database.shape_table[shape_index].remove(entity)

        color_index = hash_function(entity.color)
        database.color_table[color_index].remove(entity)
...
class Database:
    ...
    def __eq__(self, other):
        ...
        assert type(self) == type(other), "comparing Database to non-Database"
        for i in range(3):
            for j in range(5):
